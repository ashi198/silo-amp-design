import torch, os, time, pickle, core.sequence_fitness_dataset, random, mlflow, argparse
import numpy as np
...
def train_for_one_cycle(epoch: int, config: SequenceConfig, network: SequenceTransformer, network_weights: dict,
    ...
    sequence_fitness_dataset = core.sequence_fitness_dataset.SequenceFitnessDataset(config=config, 
                                           seen_protein_smiles=seen_protein_smiles)
    

    candidates = sequence_fitness_dataset.generate_dataset(network_weights, best_objective=best_objective, memory_aggressive=False)
...
def inference(epoch: int, config: SequenceConfig, network_weights: dict, logger=None, 
    ...
    with tqdm(total=config.total_peptide_count, desc="Generating peptides", unit="seq") as pbar:
        while len(generated_peptides) < config.total_peptide_count:
            ...
            sequence_fitness_dataset = core.sequence_fitness_dataset.SequenceFitnessDataset(config=config, seen_protein_smiles=seen_protein_smiles)
            initial_candidates = sequence_fitness_dataset.generate_dataset(network_weights, memory_aggressive=False)
