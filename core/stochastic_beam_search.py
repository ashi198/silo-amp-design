...
def stochastic_beam_search(
    ...
    # We also collect _all_ leaves that are encountered during the sampling process and
    ...
    while any([len(internal_states) for internal_states in batch_internal_states]):
        ...
        for batch_idx in range(batch_len):
            ...
            for node_state, node_log_prob, node_gumbel, child_log_probs in zip(
                ...
                if keep_intermediate and type(node_state) is tuple and node_state[1].is_terminable() and 0 in good_indices:
            ...
            collected_leaf_gumbels = batch_collected_leaf_gumbels[batch_idx]
            collected_leaf_log_probs = batch_collected_leaf_log_probs[batch_idx]
            collected_leaf_states = batch_collected_leaf_states[batch_idx]
    ...
    for batch_idx in range(batch_len):
        sampled_nodes = []
        #to_zip = (batch_leaf_log_probs[batch_idx], batch_leaf_gumbels[batch_idx], batch_leaf_states[batch_idx])
        #if keep_intermediate:
        to_zip = (batch_collected_leaf_log_probs[batch_idx], batch_collected_leaf_gumbels[batch_idx], batch_collected_leaf_states[batch_idx])
        for log_prob, gumbel, state in zip(*to_zip):
...
def argmax_last(x):
    ...
    if a.dtype == object:
        return np.array([int(np.asarray(row).argmax()) for row in x])
    if a.ndim <= 1:
        # (N,) case: choose your semantics; this returns zeros per entry
        return np.zeros(np.asarray(x).shape[0], dtype=int)
    return a.argmax(axis=-1)


def to_scalar_gumbels(x):
    """
    Reduce AA logits to one scalar per candidate.
    x can be:
      - (N, 20): returns (N,) via max over last axis
      - (N,):   already scalars; returned as-is
      - ragged list of arrays: handled elementwise
    """
    a = np.asarray(x, dtype=object)
    ...
    if a.ndim == 1:
        return a
    # 2D or higher: take max over the last axis (e.g., AA dim)
    return a.argmax(axis=-1)
