...
class Logger:
        ...
        if if_final: 
            ...
            if not isinstance(current_traj_epoch[0], dict):
                ...
                for trajs in current_traj_epoch:
                    traj_dict[trajs.identifier] = [''.join(trajs.seq_string), trajs.objective]
            else:
                traj_dict = {}
                for trajs in current_traj_epoch:
                    traj_dict[trajs['identifier']] = [trajs['peptide'], trajs['objective']]
    ...
    def test_pickle_save_file(self, trajectories, destination_path):
        ...
        if destination_path is not None:
            if os.path.isfile(destination_path):
                ...
                merged_seqs = sorted(merged_seqs, key=lambda x: x["objective"], reverse=self.config.max_objective)
    ...
    '''def save_results_csv(self, config, trajectories, path_csv_file):
        trajectories = sorted(trajectories, key=lambda x: x['objective'], reverse=self.config.max_objective)
        ...
        with open(path_csv_file, "a", newline="") as f:
            writer = csv.writer(f)
            if not file_exists:
                writer.writerow(["identifier", "sequence", "length", "mic_mean_score", ])
    ...
    def save_results_csv(self, config, trajectories, path_csv_file):
        trajectories = sorted(
            ...
            reverse=self.config.max_objective,
        )
