...
class SequenceDesign(BaseTrajectory):
    ...
    def __init__(self, config: SequenceConfig, inital_res: List[int]):
        """
        Parameters:
        ...
        for residue_idx in inital_res:
            if residue_idx not in self.vocabulary_residue_idcs:
                raise ValueError(f"Initial residue index {residue_idx} is outside the vocabulary")
        
            if self.residue_feasibility_mask[residue_idx]:
                raise ValueError(f"Initial residue {residue_idx} is not allowed by the vocabulary")

        self.residues = list(inital_res)
        ...
        self.history = [residue_idx + 1 for residue_idx in inital_res]
